 ...
 \ \ /\ / /|  _| |  _ \| |_) |  / _ \ | ' /\___ \ 
  \ V  V / | |___| |_) |  _ <  / ___ \| . \ ___) |
   \_/\_/  |_____|____/|_| \_\/_/   \_\_|\_\____/ 
    ...
    print(Colorate.Horizontal(cols["banner"], Center.XCenter(bn)))
    print(Colorate.Horizontal(cols["sub"], Center.XCenter("\n~ t.me/w3b4ks   discord.gg/webraksx ~")))
    print("\n")
...
def get_inpt(prompt=None):
    if prompt is None:
        prompt = f"{user}@webraks:~#"
    else:
        prompt = prompt.replace("webraks@root/", f"{user}@webraks/")
        prompt = prompt.replace("webraks@", f"{user}@webraks/")
